...
#      # set device type and create grad scalar from it
#     device_type = "cpu"
#     if device.type == "cuda":
#         device_type = "cuda"
#     elif device.type == "mps":
...
#     # gradient scalar with device type
#     scaler = torch.amp.GradScaler(enabled=(device_type == "cuda" and True))
...
#         else:
#             autocast_context = lambda: torch.amp.autocast(device_type=device.type)
        
#         for training_set, (X_train, y_train) in enumerate(train_loader_dataset):
#             optimizer.zero_grad()
...
#                 predictions = model(X_train)
#                 computed_loss = loss_function(predictions.squeeze(), y_train.float()) / gradient_accumulation
#                 loss = loss / gradient_accumulation

#             # backpropagation with gradient scalar
#             if device_type == "cuda":
#                 scaler.scale(loss).backward()
#             else:
#                 loss.backward()

#             if (training_set + 1) % gradient_accumulation == 0 or (training_set + 1) == len(train_loader_dataset):
#                 if config["stability_strategy"] == "gradient_clipping":
#                     if device_type == "cuda":
#                         scaler.unscale_(optimizer)
#                     torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
...
#             train_loss += gradient_accumulation* computed_loss.item()
#             avg_loss = train_loss / (training_set + 1)

#         # avg_train_loss = train_loss / len(train_loader_dataset)

#         # model validation
#         model.eval()
#         correct_predictions, total, validation_loss = 0, 0, 0
...
#                 # compute losss on validation set
#                 validation_loss = loss_function(validation_predictions, y_validation)
#                 validation_loss += validation_loss.item()


#                 correct_predictions += ((validation_predictions >= 0.5).float() == y_validation).sum().item()
#                 total += y_validation.size(0)

#         acc = correct_predictions / total
#         validation_loss /= len(val_loader_dataset)
#         if acc > best_accuracy:
#             best_accuracy = acc
#         print(f"Epoch {epoch+1}/{EPOCHS} | Val Loss: {validation_loss:.4f} | Val Acc: {acc:.4f}")

#     elapsed = time.time() - training_start_time
#     return {"eval_acc": best_accuracy, "eval_loss": validation_loss, "train_time": elapsed}

# def evaluate_model(model, dataloaders, config, device):
#     model.eval()
#     with torch.no_grad():
#         for X_validation, y_validation in dataloaders[config["sequence_length"]]["val"]:
#             X_validation, y_validation = X_validation.to(device), y_validation.to(device)
#             validation_predictions = model(X_validation)
#             validation_loss = loss_function(validation_predictions, y_validation)
#             validation_loss += validation_loss.item()
#             correct_predictions += ((validation_predictions >= 0.5).float() == y_validation).sum().item()
#             total += y_validation.size(0)
#     return correct_predictions / total, validation_loss / len(dataloaders[config["sequence_length"]]["val"])
